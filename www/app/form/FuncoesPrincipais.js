                ...
                EXIF.getData(arquivoFoto, function() {
                    ...
                    if (possuiData && possuiGPS) {
                        try {
                            ...
                            verificaTeresina(lon2,lat2).then(async (estaDentro) => {
                                if (estaDentro) {
...
export async function LeituraPelaCamera(Entrada){
  ...
  loopDeAnim(contador,"Verificando Localização")
...
export async function verificaTeresina(longitude, latitude) {
  ...
  //tereseina é grande ne, a pior pate foi que eu cogitei usar um for pra rodar todas as cords do TeresinaPontos.js 
